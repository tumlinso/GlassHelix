#ifndef GLASSHELIX_ASYNCHRONOUSARRAY_HH
#define GLASSHELIX_ASYNCHRONOUSARRAY_HH

//-----------------------------------------------------------------------------
// AsynchronousArray.hh
// Provides the AsynchronousArray<T> class for lazy-loading and chunked access to large binary files.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
    ...
    public:
        ...
        AsynchronousArray(std::string filename,
                          size_t recordLen,
                          size_t recordCount,
                          size_t recsPerChunk)
                : filename_(std::move(filename)), recordCount_(recordCount), recordLen_(recordLen),
...
#pragma omp parallel for schedule(dynamic, 1) default(none) shared(this)
        ...
        T *fetch_chunk(bool freeLast = false) {
            ...
            if (next < chunkCount_ && !shutting_down_) {
                if (prefetcher_.joinable())
                    prefetcher_.join();
                prefetcher_ = std::thread(&AsynchronousArray::prefetch_worker, this, next);
            }
            return ptr;
        }
        ...
        T *operator[](size_t recordIdx) {
            if (recordIdx >= recordCount_)
                throw std::out_of_range("AsynchronousArray::operator[] recordIdx out of range");
        ...
        const T *operator[](size_t recordIdx) const {
            ...
            T *buf = const_cast<AsynchronousArray *>(this)->loadChunk(ci);
...
#endif // GLASSHELIX_ASYNCHRONOUSARRAY_HH