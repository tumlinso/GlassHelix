# GH-NF1-U03: Serialize portable synthetic replay inputs
...
Explicit prerequisites: `GH-NF1-U02`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Save model expression/registered operation identity, logical data, parameter values, forcing, observation and numeric policy; exclude device pointers and ephemeral handles.

## Acceptance and negative control

1. A fresh process reconstructs the same declared calculation.
2. Replays reject missing operation definitions or incompatible versions rather than guessing.
...
Evidence kind: **executed_host_test**. Required CTest names: `gh_nf1_u03`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group u03
```
