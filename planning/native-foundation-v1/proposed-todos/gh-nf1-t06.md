# GH-NF1-T06: Publish independent qualification bundle
...
Explicit prerequisites: `GH-NF1-T05`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Record complete executed test inventory, hardware/tool/build identities and limitations, then review the mechanistic vocabulary of all public outputs.

## Acceptance and negative control

1. No fixture claims novel biology or general identifiability unsupported by its scope.
2. Remaining limitations are published, not hidden by a completion checkpoint.
...
Evidence kind: **executed_cuda_test**. Required CTest names: `gh_nf1_t06`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group t06
```
