# GH-NF1-K01: Build complete toolbox workload matrix
...
Explicit prerequisites: `GH-NF1-M30`, `GH-NF1-X05`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Sweep state/candidate batch width, n-ary count, arity, active fraction, churn, repeated steps and response directions using truth-known systems.

## Acceptance and negative control

1. Each timed computation names the same mathematical and numeric semantics as its reference.
2. A kernel microbenchmark alone cannot represent full GlassHelix use cost.
...
Evidence kind: **executed_cuda_test**. Required CTest names: `gh_nf1_k01`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group k01
```
