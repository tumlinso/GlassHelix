# GH-NF1-B01: Create native library target and remove legacy default entry
...
Replace GlassEtch as the default build through the integrator-owned root hook; retain archaeological code outside active production targets.

## Acceptance and negative control

1. A library consumer builds without running an old executable entry point.
2. No hidden legacy sparse runtime or token-state ontology enters the new library.
...
Evidence kind: **executed_host_test**. Required CTest names: `gh_nf1_b01`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group b01
```
