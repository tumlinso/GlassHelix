# GH-NF1-M90: Joint foundation closed with final peer receipt
...
Explicit prerequisites: `GH-NF1-M60`, `GH-NF1-X06`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Integrate the prerequisites onto the designated integration branch, rebuild the real linked path and verify the checkpoint contract. All NF1 work is integrated, current evidence is bound to both revisions and remaining science stays open.

## Acceptance and negative control

1. All NF1 work is integrated, current evidence is bound to both revisions and remaining science stays open.
...
Evidence kind: **integrated_executed_tests**. Required CTest names: `gh_nf1_final_source_pair`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group m90
```
