# GH-NF1-U06: Qualify replay and artifact corruption handling
...
Explicit prerequisites: `GH-NF1-U05`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Test missing files, altered hashes, wrong source/schema, unsupported custom blocks and declared numerical tolerances.

## Acceptance and negative control

1. Corruption and stale artifacts fail before consumption with no silent fallback.
2. A saved pointer or generation number alone cannot reconstruct a physical execution object.
...
Evidence kind: **executed_cuda_test**. Required CTest names: `gh_nf1_u06`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group u06
```
