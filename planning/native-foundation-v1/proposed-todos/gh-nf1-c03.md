# GH-NF1-C03: Define calculation requests and time interpretation

Status on delivery: **planned**, not claimed or implemented.
Project: `glasshelix`. First-class lane: `GH-NF1-L-C`. Workspace: isolated_merge.
Explicit prerequisites: `GH-NF1-C02`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Represent forward evaluation, discrete transition, ODE integration, selected observation, response and comparison with explicit horizon/forcing/time units.

## Acceptance and negative control

1. A discrete map is not silently multiplied by dt; an ODE vector field is not mislabeled a time-step map.
2. Arbitrary natural-language question solving and universal sufficient-state discovery are not advertised.
...
- `include/GlassHelix/core`
- `src/core`
- `tests/native_foundation/core`
- `docs/native_foundation_v1/contracts`
- `docs/native_foundation_v1/records/gh-nf1-c03.json`
...
```sh
python3 -B planning/native-foundation-v1/scripts/check_record.py --milestone GH-NF1-C03
```
