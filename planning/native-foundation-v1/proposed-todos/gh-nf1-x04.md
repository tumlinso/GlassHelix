# GH-NF1-X04: Import differential execution capability
...
Explicit prerequisites: `GH-NF1-X03`. Earlier entries in the lane queue also apply.

External prerequisite: `cellerator::CE-NF1-M40` through `NF1-CE-DIFFERENTIAL`. This is verified by this local task, not a native foreign-task dependency.

## Objective and implementation direction

Verify CE-NF1-M40 including state/parameter JVP/VJP, supported second actions and explicit unsupported cases.

## Acceptance and negative control

1. Primal generations and response support contracts agree with the GlassHelix composition.
2. A stale derivative ABI cannot unlock diagnostics.
...
```sh
python3 -B planning/native-foundation-v1/scripts/verify_receipt.py --edge NF1-CE-DIFFERENTIAL
```
