# GH-NF1-O02: Implement nonlinear observation with capabilities
...
Explicit prerequisites: `GH-NF1-O01`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Support a composed nonlinear observation and custom forward-only observation using the same numerical primitives and capability reporting.

## Acceptance and negative control

1. Nonlinear readout agrees with independent calculations and reports available derivative actions.
2. Missing derivatives are not fabricated or replaced silently.
...
Evidence kind: **executed_host_test**. Required CTest names: `gh_nf1_o02`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group o02
```
