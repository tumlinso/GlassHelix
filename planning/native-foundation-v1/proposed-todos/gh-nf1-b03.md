# GH-NF1-B03: Qualify C++20 and CUDA boundary choices
...
Explicit prerequisites: `GH-NF1-B02`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Use C++20 host code and conventional CUDA/external headers; keep named modules optional where supported rather than a dependency on a fragile compiler combination.

## Acceptance and negative control

1. Host-only and sm70 build configurations are recorded and reproducible.
2. No machine-specific absolute compiler paths, unsupported forced CUDA version or global flags override user toolchains.
...
Evidence kind: **executed_host_test**. Required CTest names: `gh_nf1_b03`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group b03
```
