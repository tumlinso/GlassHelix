# GH-NF1-S03: Lower to Cellerator numerical programs
...
Explicit prerequisites: `GH-NF1-S02`, `GH-NF1-X02`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Produce Cellerator operations and bindings with provenance to scientific definitions; reuse existing numerical contracts instead of a second interpreter/runtime.

## Acceptance and negative control

1. Native model composition reaches real Cellerator execution.
2. A host callback that computes the entire model while reporting CUDA capability cannot pass.
...
Evidence kind: **executed_cuda_test**. Required CTest names: `gh_nf1_s03`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group s03
```
