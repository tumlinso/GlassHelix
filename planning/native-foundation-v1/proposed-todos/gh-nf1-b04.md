# GH-NF1-B04: Wire tests, examples and package metadata
...
Explicit prerequisites: `GH-NF1-B03`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Add opt-in foundation examples, independent references and test registration with source/build identity; keep missing real backend a failure.

## Acceptance and negative control

1. Reference-only demonstration and real linked demonstration are distinct targets.
2. Syntax-only declaration headers are never on the normal production include path.
...
Evidence kind: **executed_host_test**. Required CTest names: `gh_nf1_b04`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group b04
```
