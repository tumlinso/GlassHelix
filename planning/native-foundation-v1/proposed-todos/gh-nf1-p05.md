# GH-NF1-P05: Qualify optional device interchange or reject it honestly

Status on delivery: **planned**, not claimed or implemented.
Project: `glasshelix`. First-class lane: `GH-NF1-L-P`. Workspace: isolated_merge.
Explicit prerequisites: `GH-NF1-P04`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Use a checked lifetime/stream protocol for any enabled GPU interchange; a host-first release is acceptable only with explicit unsupported device interchange.

## Acceptance and negative control

1. Advertised device borrowing is tested for stream and lifetime correctness.
2. A raw device pointer with no readiness contract is not sufficient for zero-copy support.
...
- `bindings/python`
- `python/glasshelix`
- `tests/native_foundation/python`
- `docs/native_foundation_v1/python`
...
Evidence kind: **executed_cuda_test**. Required CTest names: `gh_nf1_p05`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group p05
```
