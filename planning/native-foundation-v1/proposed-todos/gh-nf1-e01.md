# GH-NF1-E01: Implement explicit finite candidate state with weights
...
Explicit prerequisites: `GH-NF1-M20`, `GH-NF1-O04`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Hold supplied state/mechanism hypotheses and weights separately from physical state; preserve joint candidate identity and align comparable outputs.

## Acceptance and negative control

1. Equal observations can leave multiple candidates alive.
2. A finite supplied family is not claimed to enumerate all biological mechanisms.
...
Evidence kind: **executed_host_test**. Required CTest names: `gh_nf1_e01`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group e01
```
