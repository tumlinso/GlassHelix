# GH-NF1-C01: Define quantity roles over Cellerator identities
...
Explicit prerequisites: `GH-NF1-M00`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Reuse domain/order/structure identities while distinguishing dynamical state, observation, inferential information and parameters; allow optional biological naming.

## Acceptance and negative control

1. Equal shapes with different domain/order/role metadata are not interchangeable.
2. A field need not be named as a molecule, and a stable software ID is not evidence of molecular identifiability.
...
- `docs/native_foundation_v1/records/gh-nf1-c01.json`
...
```sh
python3 -B planning/native-foundation-v1/scripts/check_record.py --milestone GH-NF1-C01
```
