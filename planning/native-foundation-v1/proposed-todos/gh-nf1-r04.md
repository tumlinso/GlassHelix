# GH-NF1-R04: Implement limited bound propagation and decision guards

Status on delivery: **planned**, not claimed or implemented.
Project: `glasshelix`. First-class lane: `GH-NF1-L-R`. Workspace: isolated_merge.
Explicit prerequisites: `GH-NF1-R03`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Propagate error only when supplied Lipschitz/operator bounds and local error controls apply; otherwise report measured error and unavailable guarantees.

## Acceptance and negative control

1. A known bounded linear fixture satisfies its propagated bound and a threshold case can be unresolved.
2. Empirical Jacobian norms are not silently promoted to global bounds.
...
- `include/GlassHelix/reduction`
- `src/reduction`
- `tests/native_foundation/reduction`
...
Evidence kind: **executed_cuda_test**. Required CTest names: `gh_nf1_r04`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group r04
```
