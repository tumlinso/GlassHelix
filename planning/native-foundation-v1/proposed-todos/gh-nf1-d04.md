# GH-NF1-D04: Implement batched rollout and observation-time selection
...
Explicit prerequisites: `GH-NF1-D03`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Keep state on device through repeated stages, selecting explicit output times without full canonicalization after every step.

## Acceptance and negative control

1. Width tails and independent trajectories preserve entity/candidate identity.
2. Coupled cells or candidate states cannot be treated as independent to enlarge a matrix dimension.
...
Evidence kind: **executed_cuda_test**. Required CTest names: `gh_nf1_d04`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group d04
```
