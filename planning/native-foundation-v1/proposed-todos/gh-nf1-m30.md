# GH-NF1-M30: Observation, inference and interrogation integrated

Status on delivery: **planned**, not claimed or implemented.
Project: `glasshelix`. First-class lane: `GH-NF1-L-M`. Workspace: isolated_merge.
Explicit prerequisites: `GH-NF1-M20`, `GH-NF1-O06`, `GH-NF1-E05`, `GH-NF1-Q07`, `GH-NF1-R05`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Integrate the prerequisites onto the designated integration branch, rebuild the real linked path and verify the checkpoint contract. Explicit observation/inference distinction, response and local null instruments work with correct provenance.

## Acceptance and negative control

1. Explicit observation/inference distinction, response and local null instruments work with correct provenance.
...
- `CMakeLists.txt`
- `docs/native_foundation_v1/integration`
...
Evidence kind: **integrated_executed_tests**. Required CTest names: `gh_nf1_ambiguity_refinement_cuda`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group m30
```
