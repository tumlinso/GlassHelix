# GH-NF1-U02: Implement run and failure inspection
...
Explicit prerequisites: `GH-NF1-U01`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Export source/model/parameter/state generations, stage provenance, activity decisions, timings and readiness status.

## Acceptance and negative control

1. Failed or incomplete execution remains distinguishable from empty but valid output.
2. Enqueue counters cannot certify numerical completion.
...
Evidence kind: **executed_host_test**. Required CTest names: `gh_nf1_u02`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group u02
```
