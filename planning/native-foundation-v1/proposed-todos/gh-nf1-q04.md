# GH-NF1-Q04: Compute local null directions with established linear algebra

Status on delivery: **planned**, not claimed or implemented.
Project: `glasshelix`. First-class lane: `GH-NF1-L-Q`. Workspace: isolated_merge.
Explicit prerequisites: `GH-NF1-Q03`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Decompose the sensitivity itself using a qualified SVD or rank-revealing method, report threshold/residual/domain and avoid unnecessary normal equations.

## Acceptance and negative control

1. The a+b fixture detects its unresolved contrast and an added readout can remove that local ambiguity.
2. Local near-null singular values are not a global structural-identifiability theorem.
...
- `include/GlassHelix/interrogation`
- `src/interrogation`
- `tests/native_foundation/interrogation`
...
Evidence kind: **executed_cuda_test**. Required CTest names: `gh_nf1_q04`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group q04
```
