# GH-NF1-X01: Import the developmental Cellerator contracts
...
External prerequisite: `cellerator::CE-NF1-M10` through `NF1-CE-CONTRACTS`. This is verified by this local task, not a native foreign-task dependency.

## Objective and implementation direction

Verify CE-NF1-M10 producer receipt against authority identity, contract version/hashes and integrated source before consumer assumptions are frozen.

## Acceptance and negative control

1. Receipt distinguishes an agreed interface from an executed implementation.
2. A draft or changed contract does not unlock an execution claim.
...
```sh
python3 -B planning/native-foundation-v1/scripts/verify_receipt.py --edge NF1-CE-CONTRACTS
```
