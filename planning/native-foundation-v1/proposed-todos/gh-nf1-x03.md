# GH-NF1-X03: Import n-ary and support execution capability
...
Explicit prerequisites: `GH-NF1-X02`. Earlier entries in the lane queue also apply.

External prerequisite: `cellerator::CE-NF1-M30` through `NF1-CE-NARY`. This is verified by this local task, not a native foreign-task dependency.

## Objective and implementation direction

Verify CE-NF1-M30 including native nonlinear evaluator, activity semantics, tails and topology-epoch handling.

## Acceptance and negative control

1. Missing or changed capability keeps dependent work blocked.
2. Pairwise-only approximation and CPU surrogate do not qualify as n-ary GPU execution.
...
```sh
python3 -B planning/native-foundation-v1/scripts/verify_receipt.py --edge NF1-CE-NARY
```
