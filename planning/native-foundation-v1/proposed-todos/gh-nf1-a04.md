# GH-NF1-A04: Publish autonomy and recovery charter
...
Explicit prerequisites: `GH-NF1-A03`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Record authority-backed scope transfer, interface revision, early integration, intermittent push, worktree recovery and resource rules for all lanes.

## Acceptance and negative control

1. The controller can autonomously select ready tasks, revise scopes through the workflow and continue without per-task human prompts.
2. Autonomy does not waive unknown-file preservation, scientific correctness, resource leases or the no-fabricated-evidence rule.
...
- `docs/native_foundation_v1/records/gh-nf1-a04.json`
...
```sh
python3 -B planning/native-foundation-v1/scripts/check_record.py --milestone GH-NF1-A04
```
