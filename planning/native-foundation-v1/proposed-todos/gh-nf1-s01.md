# GH-NF1-S01: Implement system and mechanism assembly

Status on delivery: **planned**, not claimed or implemented.
Project: `glasshelix`. First-class lane: `GH-NF1-L-S`. Workspace: isolated_merge.
Explicit prerequisites: `GH-NF1-C07`, `GH-NF1-X01`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Build scientific descriptions from inspectable arithmetic and compiled blocks with ordered n-ary inputs, explicit outputs and reusable immutable definitions.

## Acceptance and negative control

1. The example triad is represented as one joint nonlinear mechanism, not pairwise additive edges.
2. Assembly never silently invents a biological actor for an execution grouping.
...
- `include/GlassHelix/system`
- `src/system`
- `tests/native_foundation/system`
...
Evidence kind: **executed_host_test**. Required CTest names: `gh_nf1_s01`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group s01
```
