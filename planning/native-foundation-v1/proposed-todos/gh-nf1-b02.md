# GH-NF1-B02: Bind the peer source revision and consumer targets
...
Explicit prerequisites: `GH-NF1-B01`, `GH-NF1-X02`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Consume explicit Cellerator root/revision and the imported CE-NF1 target receipt; avoid copying kernels or using a stale historical submodule by accident.

## Acceptance and negative control

1. Standalone consumer proves headers and actual library symbols come from the qualified revision.
2. Including implementation .cu source or forcing Torch ownership fails the build-boundary check.
...
Evidence kind: **executed_host_test**. Required CTest names: `gh_nf1_b02`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group b02
```
