# GH-NF1-R02: Evaluate closure and response residuals
...
Explicit prerequisites: `GH-NF1-R01`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Compute supplied reduction closure checks on controlled systems with declared domain, forcing and derivative availability.

## Acceptance and negative control

1. An intentionally nonclosed reduction is detected even if it predicts one present output.
2. Sampled closure residual is not labeled a global proof without additional assumptions.
...
Evidence kind: **executed_host_test**. Required CTest names: `gh_nf1_r02`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group r02
```
