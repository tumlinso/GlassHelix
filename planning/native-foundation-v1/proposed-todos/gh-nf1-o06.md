# GH-NF1-O06: Qualify observation contract
...
Explicit prerequisites: `GH-NF1-O05`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Integrate host/CUDA observation, masks, likelihood and unsupported-capability cases.

## Acceptance and negative control

1. The same state can produce distinct observation regimes without rebuilding its biological identity.
2. A residual alone cannot be mislabeled a normalized likelihood or an identified inverse.
...
Evidence kind: **executed_cuda_test**. Required CTest names: `gh_nf1_o06`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group o06
```
