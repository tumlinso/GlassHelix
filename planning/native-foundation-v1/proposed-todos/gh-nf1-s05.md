# GH-NF1-S05: Implement correct shared-result eligibility
...
Explicit prerequisites: `GH-NF1-S04`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Reuse outputs only when all effect-relevant inputs, generations, forcing, time, activity and numeric policy agree; share code/support more broadly where legal.

## Acceptance and negative control

1. Deliberately modified same-address values invalidate result reuse.
2. Similar latent labels or equal output shape do not prove equivalence.
...
Evidence kind: **executed_cuda_test**. Required CTest names: `gh_nf1_s05`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group s05
```
