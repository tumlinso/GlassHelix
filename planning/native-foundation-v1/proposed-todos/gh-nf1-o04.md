# GH-NF1-O04: Implement elementary noise and likelihood instruments
...
Explicit prerequisites: `GH-NF1-O03`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Provide deterministic mean/readout and an explicit simple noise likelihood/sampler for truth fixtures, with reproducible observation RNG distinct from process noise.

## Acceptance and negative control

1. Likelihood and sampling checks match the specified model.
2. Data noise does not silently alter deterministic state dynamics or imply a universal Gaussian observation ontology.
...
Evidence kind: **executed_host_test**. Required CTest names: `gh_nf1_o04`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group o04
```
