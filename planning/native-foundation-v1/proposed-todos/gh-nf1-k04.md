# GH-NF1-K04: Publish numerical-performance tradeoffs and requests
...
Explicit prerequisites: `GH-NF1-K03`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Compare f32 baseline and qualified mixed/approximate paths, producing targeted Cellerator feedback backed by actual workload evidence.

## Acceptance and negative control

1. Negative results remain visible and new primitive requests state reusable biological structure.
2. GlassHelix does not create duplicate low-level kernels to avoid an owner change.
...
Evidence kind: **executed_cuda_test**. Required CTest names: `gh_nf1_k04`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group k04
```
