# GH-NF1-E03: Update weights from recorded observation likelihood
...
Explicit prerequisites: `GH-NF1-E02`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Apply stable normalized likelihood updates with explicit zero-total-evidence failure and missing-channel handling.

## Acceptance and negative control

1. A discriminating second modality changes weights while the initial identical observation does not.
2. Candidate rejection, underflow and absent observations cannot silently produce certainty.
...
Evidence kind: **executed_cuda_test**. Required CTest names: `gh_nf1_e03`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group e03
```
