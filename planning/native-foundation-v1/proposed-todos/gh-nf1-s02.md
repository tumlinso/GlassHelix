# GH-NF1-S02: Validate domain, role, effect and algebraic closure
...
Explicit prerequisites: `GH-NF1-S01`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Check schema identities, output initialization, declared dependencies and effect ordering; require explicit solver support for algebraic cycles.

## Acceptance and negative control

1. Invalid axes, repeated ownership and unsupported algebraic loops fail before preparation.
2. A cycle is not resolved by whichever kernel happens to execute first.
...
Evidence kind: **executed_host_test**. Required CTest names: `gh_nf1_s02`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group s02
```
