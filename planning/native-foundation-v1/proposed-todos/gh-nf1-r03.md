# GH-NF1-R03: Track support-specific approximation claims
...
Explicit prerequisites: `GH-NF1-R02`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Record dropped support, numeric conversions, integration error and selected response support separately with certified/empirical/unassessed status.

## Acceptance and negative control

1. Different error sources remain distinguishable in a result record.
2. Near-zero fitting coefficients are not scientific evidence of absent interactions.
...
Evidence kind: **executed_cuda_test**. Required CTest names: `gh_nf1_r03`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group r03
```
