# GH-NF1-X06: Import final Cellerator closure receipt

Status on delivery: **planned**, not claimed or implemented.
Project: `glasshelix`. First-class lane: `GH-NF1-L-X`. Workspace: isolated_merge.
Explicit prerequisites: `GH-NF1-X05`. Earlier entries in the lane queue also apply.

External prerequisite: `cellerator::CE-NF1-M90` through `NF1-CE-FINAL`. This is verified by this local task, not a native foreign-task dependency.

## Objective and implementation direction

After GH-NF1-M60 publishes consumer acceptance and CE-NF1-M90 closes, import the final peer receipt and verify both integrated main revisions.

## Acceptance and negative control

1. GlassHelix final closure is bound to the qualified final pair.
2. Merely reaching Cellerator's old RU1 checkpoint or a different NF1 build cannot close this dependency.
...
- `docs/native_foundation_v1/imports`
...
```sh
python3 -B planning/native-foundation-v1/scripts/verify_receipt.py --edge NF1-CE-FINAL
```
