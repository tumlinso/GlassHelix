# GH-NF1-P03: Handle asynchronous buffer and exception lifetime
...
Explicit prerequisites: `GH-NF1-P02`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Retain borrowed owners through completion, release the GIL around native work where safe and translate errors without losing source/status.

## Acceptance and negative control

1. Dropping a Python reference cannot free a still-consumed buffer.
2. Interpreter callback errors and device failure cannot produce valid completion tickets.
...
Evidence kind: **executed_cuda_test**. Required CTest names: `gh_nf1_p03`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group p03
```
