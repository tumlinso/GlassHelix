# GH-NF1-M60: Real GlassHelix consumer acceptance published
...
Explicit prerequisites: `GH-NF1-M40`, `GH-NF1-T06`, `GH-NF1-K05`, `GH-NF1-X05`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Integrate the prerequisites onto the designated integration branch, rebuild the real linked path and verify the checkpoint contract. Publish the real-consumer receipt required by CE-NF1-X01, without waiting for Cellerator final closure.

## Acceptance and negative control

1. Publish the real-consumer receipt required by CE-NF1-X01, without waiting for Cellerator final closure.
...
Evidence kind: **integrated_executed_tests**. Required CTest names: `gh_nf1_full_consumer_acceptance`, `gh_nf1_complete_cost_receipt`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group m60
```
