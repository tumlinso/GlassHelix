# GH-NF1-Q07: Qualify diagnostic readout and provenance
...
Explicit prerequisites: `GH-NF1-Q06`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Export numeric values, scales, primal/source generations, candidate alignment and exact-versus-local status.

## Acceptance and negative control

1. A stale parameter or structure generation cannot be interrogated as a current result.
2. Numerical nulls cannot automatically prune mechanisms from a different future question.
...
Evidence kind: **executed_cuda_test**. Required CTest names: `gh_nf1_q07`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group q07
```
