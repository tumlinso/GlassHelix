# GH-NF1-O03: Implement missingness and observation provenance
...
Explicit prerequisites: `GH-NF1-O02`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Preserve masks, measured zeros, units, timestamps and individual/population record kind.

## Acceptance and negative control

1. Masking a channel removes its evidence contribution without overwriting its state.
2. Population observations are rejected by individual-history-only instruments.
...
Evidence kind: **executed_host_test**. Required CTest names: `gh_nf1_o03`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group o03
```
