# GH-NF1-R01: Represent supplied exact and approximate reductions
...
Explicit prerequisites: `GH-NF1-M20`, `GH-NF1-X04`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Accept a declared map from full to reduced state with preservation obligations and compatible observation roles; do not search a universal minimal quotient.

## Acceptance and negative control

1. Known equal-decay aggregation preserves selected outputs under the admitted regime.
2. Answer sufficiency alone cannot be claimed as closed state evolution.
...
Evidence kind: **executed_host_test**. Required CTest names: `gh_nf1_r01`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group r01
```
