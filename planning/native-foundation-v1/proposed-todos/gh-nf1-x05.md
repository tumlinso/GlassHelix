# GH-NF1-X05: Import qualified consumer-ready bundle
...
Explicit prerequisites: `GH-NF1-X04`. Earlier entries in the lane queue also apply.

External prerequisite: `cellerator::CE-NF1-M50` through `NF1-CE-READY`. This is verified by this local task, not a native foreign-task dependency.

## Objective and implementation direction

Verify CE-NF1-M50 with independent tests, relevant regressions and numerical/performance limitations.

## Acceptance and negative control

1. All prior imported contracts are still compatible with the consumed revision.
2. A future Cellerator final closure is not required here, avoiding cross-project deadlock.
...
```sh
python3 -B planning/native-foundation-v1/scripts/verify_receipt.py --edge NF1-CE-READY
```
