# GH-NF1-T04: Qualify integrated real CPU/CUDA path
...
Explicit prerequisites: `GH-NF1-T03`, `GH-NF1-M30`, `GH-NF1-X05`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Run truth fixtures through the linked native system, observation and derivative paths using the qualified peer receipt.

## Acceptance and negative control

1. Real device required for GPU cases; CPU f64 truth remains independent.
2. No skipped GPU tests, mocked backend or contract-only headers can satisfy integrated acceptance.
...
Evidence kind: **executed_cuda_test**. Required CTest names: `gh_nf1_t04`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group t04
```
