# GH-NF1-D02: Implement vector-field evaluation and RK4
...
Explicit prerequisites: `GH-NF1-D01`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Compose fixed-step RK4 stages from Cellerator operations without interpreting dt as biology or introducing a hidden learned integrator.

## Acceptance and negative control

1. Linear and nonlinear truth fixtures exhibit correct one-step output and fourth-order convergence where applicable.
2. Stiff/adaptive/event-aware guarantees are not claimed; unsupported requests remain clear.
...
Evidence kind: **executed_host_test**. Required CTest names: `gh_nf1_d02`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group d02
```
