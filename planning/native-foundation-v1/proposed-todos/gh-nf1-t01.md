# GH-NF1-T01: Author truth-known systems independent of production lowering
...
Implement explicit f64 linear, nonlinear triad, hidden-state and reduction formulas with independently constructed identities and maps.

## Acceptance and negative control

1. Deliberately incorrect production variants are detectable.
2. Shared optimized kernels cannot serve as their own only oracle.
...
Evidence kind: **executed_host_test**. Required CTest names: `gh_nf1_t01`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group t01
```
