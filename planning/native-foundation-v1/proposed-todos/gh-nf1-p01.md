# GH-NF1-P01: Implement thin native bindings over the common library
...
Explicit prerequisites: `GH-NF1-M30`. Earlier entries in the lane queue also apply.

## Objective and implementation direction

Expose construction, preparation, execution, observation and inspection through one chosen small binding layer; validate the previously proposed nanobind choice against the actual build.

## Acceptance and negative control

1. Python and C++ requests reach identical native semantics.
2. No Python numerical reimplementation or Torch-owned runtime becomes required.
...
Evidence kind: **executed_host_test**. Required CTest names: `gh_nf1_p01`.

```sh
python3 -B planning/native-foundation-v1/scripts/run_gate.py --group p01
```
